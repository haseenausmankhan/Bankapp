...
const verifyToken = async (req, res, next) => {
    ...
    try {
        ...
        const tokenEntry = await database.get('SELECT * FROM tokens WHERE token = ? AND user_id = ?', [token, decoded.id]);
...
app.get('/api/me', verifyToken, async (req, res) => {
    const user = await req.db.get('SELECT id, name, email, balance FROM users WHERE id = ?', [req.user.id]);
    res.json({ user });
});
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        await req.db.run('INSERT INTO tokens (user_id, token, expires_at) VALUES (?, ?, ?)', [userId, token, expiresAt]);
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        const user = await req.db.get('SELECT * FROM users WHERE email = ? AND password = ?', [email, password]);
        if (user) {
            const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: '1h' });
            const expiresAt = new Date(Date.now() + 3600000).toISOString();

            await req.db.run('INSERT INTO tokens (user_id, token, expires_at) VALUES (?, ?, ?)', [user.id, token, expiresAt]);
...
app.post('/api/logout', verifyToken, async (req, res) => {
    const token = req.cookies.token;
    await req.db.run('DELETE FROM tokens WHERE token = ?', [token]);
...
app.get('/api/balance', verifyToken, async (req, res) => {
    try {
        const user = await req.db.get('SELECT balance FROM users WHERE id = ?', [req.user.id]);
...
app.post('/api/transfer', verifyToken, async (req, res) => {
    ...
    try {
        const sender = await req.db.get('SELECT balance, name FROM users WHERE id = ?', [fromId]);
        const recipient = await req.db.get('SELECT id, name FROM users WHERE email = ?', [toEmail]);
        ...
        await req.db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [transferAmount, fromId]);
        await req.db.run('UPDATE users SET balance = balance + ? WHERE id = ?', [transferAmount, recipient.id]);

        // Log transactions
        await req.db.run('INSERT INTO transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)',
...
app.post('/api/deposit', verifyToken, async (req, res) => {
    ...
    try {
        await req.db.run('UPDATE users SET balance = balance + ? WHERE id = ?', [depositAmount, req.user.id]);
        await req.db.run('INSERT INTO transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)',
...
app.post('/api/withdraw', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await req.db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [withdrawAmount, req.user.id]);
        await req.db.run('INSERT INTO transactions (user_id, type, amount, description) VALUES (?, ?, ?, ?)',
...
app.get('/api/transactions', verifyToken, async (req, res) => {
    try {
        const transactions = await req.db.all('SELECT * FROM transactions WHERE user_id = ? ORDER BY date DESC LIMIT 50', [req.user.id]);
